...
function autoComplete(type) {
  ...
  const box = document.getElementById(`autocomplete-${type}`); // ID 수정 (중요!)

  if (!keyword) {
    box.classList.add("hidden"); // hidden 클래스 추가
    return;
  }
  ...
  ps.keywordSearch(keyword, (data, status) => {
    ...
    box.innerHTML = "";
    box.classList.remove("hidden"); // hidden 클래스 제거하여 노출
    ...
    data.forEach(place => {
      ...
      item.onmousedown = (e) => {
        ...
        box.classList.add("hidden"); // 선택 후 다시 숨김
